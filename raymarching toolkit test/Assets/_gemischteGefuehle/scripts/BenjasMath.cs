...
public class BenjasMath : MonoBehaviour{
	...
	public static Vector3 VectorComponentProduct (Vector3 A ,Vector3 B )
	{
		A.x *= B.x;
		A.y *= B.y;
		A.z *= B.z;
...
*/
    ...
    public static float mapSteps(float inputValue, float[] inputBoarders, float[] outputBoarders)
    {
        ...
        if (inputValue < inputBoarders[0])
            return outputBoarders[0];
        for (int i = 0; i < inputBoarders.Length - 1; i++)
        {
            if (inputValue >= inputBoarders[i] && inputValue <= inputBoarders[i + 1])
            {
                if (i >= outputBoarders.Length - 1)
                {
                    return outputBoarders[outputBoarders.Length - 1];
                }
                else
                {
                    return BenjasMath.map(inputValue,
                        inputBoarders[i], inputBoarders[i + 1],
                        outputBoarders[i], outputBoarders[i + 1]);
                }
            }
        }
